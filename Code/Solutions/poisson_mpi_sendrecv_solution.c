...
int main(int argc, char *argv[])
{
  ...
  while(l2 > eps) {
    ...
    if (top_neighbor<mpi_size) {
      MPI_Sendrecv( u[num_local_lines], numpoints, MPI_DOUBLE, top_neighbor, 0,
                    utmp[num_local_lines+1], numpoints, MPI_DOUBLE, top_neighbor, 0,
                    MPI_COMM_WORLD, &status );
    }
    int bottom_neighbor = mpi_rank-1;
    if (bottom_neighbor>=0) {
      MPI_Sendrecv( u[1], numpoints, MPI_DOUBLE, bottom_neighbor, 0,
                    utmp[0], numpoints, MPI_DOUBLE, bottom_neighbor, 0,
                    MPI_COMM_WORLD, &status );
    }
